...
$query_for_images = "SELECT images from package_images where pname = '$pname'";
    ...
    <br>
    <?php echo "<div style='margin-top: -7rem; margin-left: 6.5rem; font-size: 2.5rem; color: #cadefc'>".$all_fetch['price']."</div>"; ?>   <br>

    <?php echo "<div style='margin-left: 5rem; color: #cadefc; font-size: 1.5rem; margin-top: -1rem'>".$all_fetch['duration']."</div>"; ?>
    <br>
    <?php
   ...
   ?>
    ...
    <?php echo"<div style='margin-left: 24rem; margin-right: 1.5rem; margin-top: -22rem;  font-family: 'Source Sans Pro', 'Arial', sans-serif;'>". $all_fetch['description']."</div>"; ?>
    <br>

    <div style="margin-left: 40rem; margin-top: 17rem; width: 10rem">
  <a href="user_login.php?pname=<?php echo urlencode($pname); ?>" >
    <button style="width: 15rem; height: 2rem; font-size: 1.2rem; background-color: #bc2525; color: white; border-radius: 5px; cursor: pointer">Book Now</button>
  </a>
    </div>
